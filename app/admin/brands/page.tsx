'use client'

import { useState, useEffect } from 'react'
import { Button } from '@/components/ui/button'
import { Card, CardContent } from '@/components/ui/card'
import ImageUploader from '@/components/admin/ImageUploader'

interface Brand {
  id: string
  isim: string
  logoURL: string
  sira: number
}

export default function BrandsAdmin() {
  const [brands, setBrands] = useState<Brand[]>([])
  const [editingBrand, setEditingBrand] = useState<Partial<Brand> | null>(null)
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [message, setMessage] = useState('')

  useEffect(() => {
    fetchBrands()
  }, [])

  const fetchBrands = async () => {
    try {
      const res = await fetch('/api/sections/brands')
      if (res.ok) {
        const json = await res.json()
        setBrands(json || [])
      }
    } catch (error) {
      console.error('Veri yüklenemedi:', error)
    } finally {
      setLoading(false)
    }
  }

  const handleSave = async () => {
    if (!editingBrand) return
    
    setSaving(true)
    setMessage('')

    try {
      const method = editingBrand.id ? 'PUT' : 'POST'
      const res = await fetch('/api/sections/brands', {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(editingBrand)
      })

      if (res.ok) {
        setMessage('Başarıyla kaydedildi!')
        setEditingBrand(null)
        fetchBrands()
      } else {
        setMessage('Kaydetme hatası oluştu')
      }
    } catch (error) {
      setMessage('Bir hata oluştu')
    } finally {
      setSaving(false)
    }
  }

  const handleDelete = async (id: string) => {
    if (!confirm('Bu markayı silmek istediğinize emin misiniz?')) return

    try {
      const res = await fetch(`/api/sections/brands?id=${id}`, {
        method: 'DELETE'
      })

      if (res.ok) {
        setMessage('Marka silindi!')
        fetchBrands()
      }
    } catch (error) {
      setMessage('Silme hatası oluştu')
    }
  }

  const startEdit = (brand: Brand) => {
    setEditingBrand(brand)
  }

  const startNew = () => {
    setEditingBrand({
      isim: '',
      logoURL: '',
      sira: brands.length + 1
    })
  }

  if (loading) {
    return <div className="text-center text-xl font-semibold text-gray-800 py-8">Yükleniyor...</div>
  }

  return (
    <div>
      <div className="mb-8 flex justify-between items-center">
        <div>
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Markalar</h1>
          <p className="text-gray-700 text-lg font-medium">Ortak markalarınızı yönetin</p>
        </div>
        <Button onClick={startNew} className="bg-blue-600 hover:bg-blue-700">
          <i className="fas fa-plus mr-2"></i>
          Yeni Marka
        </Button>
      </div>

      {message && (
        <div className={`mb-4 p-4 rounded-lg ${message.includes('Başarıyla') || message.includes('silindi') ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
          {message}
        </div>
      )}

      {editingBrand && (
        <Card className="mb-6">
          <CardContent className="p-6">
            <h3 className="text-2xl font-bold mb-4 text-gray-900">
              {editingBrand.id ? 'Markayı Düzenle' : 'Yeni Marka Ekle'}
            </h3>
            
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-semibold text-gray-800 mb-2">
                  Marka Adı
                </label>
                <input
                  type="text"
                  value={editingBrand.isim || ''}
                  onChange={(e) => setEditingBrand({ ...editingBrand, isim: e.target.value })}
                  className="w-full px-4 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-gray-800 font-medium"
                />
              </div>

              <ImageUploader
                value={editingBrand.logoURL || ''}
                onChange={(url) => setEditingBrand({ ...editingBrand, logoURL: url })}
                label="Logo URL"
              />

              <div>
                <label className="block text-sm font-semibold text-gray-800 mb-2">
                  Sıra
                </label>
                <input
                  type="number"
                  value={editingBrand.sira || 0}
                  onChange={(e) => setEditingBrand({ ...editingBrand, sira: parseInt(e.target.value) })}
                  className="w-full px-4 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-gray-800 font-medium"
                />
              </div>

              <div className="flex gap-2">
                <Button
                  onClick={handleSave}
                  disabled={saving}
                  className="flex-1 bg-blue-600 hover:bg-blue-700"
                >
                  {saving ? 'Kaydediliyor...' : 'Kaydet'}
                </Button>
                <Button
                  onClick={() => setEditingBrand(null)}
                  className="bg-gray-500 hover:bg-gray-600"
                >
                  İptal
                </Button>
              </div>
            </div>
          </CardContent>
        </Card>
      )}

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {brands.map((brand) => (
          <Card key={brand.id}>
            <CardContent className="p-4">
              <div className="aspect-square relative bg-gray-100 rounded-lg mb-4 overflow-hidden">
                <img
                  src={brand.logoURL}
                  alt={brand.isim}
                  className="w-full h-full object-contain"
                />
              </div>
              <h3 className="font-bold text-lg mb-2 text-gray-900">{brand.isim}</h3>
              <p className="text-sm text-gray-700 font-semibold mb-4">Sıra: {brand.sira}</p>
              <div className="flex gap-2">
                <Button
                  onClick={() => startEdit(brand)}
                  className="flex-1 bg-blue-600 hover:bg-blue-700"
                  size="sm"
                >
                  Düzenle
                </Button>
                <Button
                  onClick={() => handleDelete(brand.id)}
                  className="bg-red-500 hover:bg-red-600"
                  size="sm"
                >
                  Sil
                </Button>
              </div>
            </CardContent>
          </Card>
        ))}
      </div>
    </div>
  )
}

